...
The cluster is a direct parallel to the (now destroyed — see `project_eks_burndown_2026-04-21.md`) EKS stack. Same dual-node-pool shape (cheap Spot default + dedicated on-demand database pool), same single-AZ DB-pool pinning to avoid CNPG PV orphaning, same public-api + public-lb ergonomics. Anything that falls out of the "port EKS patterns first" mandate from the epic (see `feedback_aks_port_preference.md`) gets called out inline.
...
|---|---|---|
| Database-pool AZ | Zone `1` | Most broadly supported SKU set in southcentralus; greenfield, no pre-existing PV to match |
...
| Default-pool SKU fallback | None — hardcoded `Standard_D2ps_v5` | Multi-arch image work is out of scope for this phase; let `tofu apply` fail loudly if ARM Spot unavailable in southcentralus rather than silently downgrading to amd64 |
...
- `azurerm_kubernetes_cluster "main"` — Azure CNI Overlay, AAD-RBAC enabled, `SystemAssigned` identity.
  - Default pool: `Standard_D2ps_v5` (ARM), Spot with `Delete` eviction and `spot_max_price = -1`, autoscale 1–3, 50 GB OS disk, attached to `azurerm_subnet.aks_nodes`.
...
- `azurerm_kubernetes_cluster_node_pool "database"` — `Standard_D2pls_v5`, `Regular` priority, `zones = [var.database_pool_zone]` (default `"1"`), `node_count = 1`, 30 GB OS disk, label `workload=database`, taint `workload=database:NoSchedule`.
...
- Add `variable "database_pool_zone"` (default `"1"`).
...
- `terraform/azure/network.tf` — no changes needed. AKS is attached to the existing `aks_nodes` subnet. Overlay means pods don't consume subnet IPs, so the `/24` carved out in TRA-436 (256 IPs → ~251 usable for nodes) is ample for a ≤3-node default pool + 1-node DB pool.
...
1. `just azure`
   → azure-new resources created in order:
     ...
     - azurerm_kubernetes_cluster.main (creates kubelet_identity)
     - azurerm_kubernetes_cluster_node_pool.database
...
No Helm, no kubectl in this phase — TRA-438 is where the cluster gets populated.
...
- [ ] `az aks get-credentials --resource-group rg-trakrf-demo-ussc --name aks-trakrf-demo-ussc` succeeds (command name is the actual rendered one).
- [ ] `kubectl cluster-info` returns control-plane + coredns endpoints.
- [ ] `kubectl get nodes` shows 1 default node + 1 database node; database node has the `workload=database` label and `workload=database:NoSchedule` taint.
...
- **ARM Spot availability in southcentralus**: pre-flight with `az vm list-skus -l southcentralus --resource-type virtualMachines --query "[?name=='Standard_D2ps_v5']" -o table`. If unavailable, the apply fails; decision is to *not* bake in a fallback SKU for this phase.
